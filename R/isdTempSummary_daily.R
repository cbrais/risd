#' Daily temperature summaries from NOAA's Integrated Surface Database (ISD)
#'
...
#' @import tidyverse
#' @import lubridate
#' @param isd_station_data data frame containing ISD station data, as outputed by rnoaa's \code{isd()} or risd's \code{getStationData_single()} or \code{getStationData_all()}.
#' @param f option to convert to Fahrenheit (default is \code{FALSE}).
#' @return Returns a data frame with the daily temperature summaries for the station data.
#' @examples
#' stmattTempSumm <-  isdTempSummary_daily(stmatthewsIsland1940)           # Data retrieved with getStationData_single()
#' stmattTempSummF <- isdTempSummary_daily(stmatthewsIsland1940, f = TRUE) # Same as above, but in Fahrenheit
#' icelandTempSumm <- isdTempSummary_daily(icelandSubset)                  # Data retrieved with getStationData_all()
#' @export
isdTempSummary_daily <- function(isd_station_data, f = FALSE) {
  ...
  mean <- isd_station_data %>%
    group_by(usaf_wban, year(date), month(date), day(date)) %>%
  ...
  temp_summary <- temp_summary %>%
    add_column(date = as.POSIXct(strptime(paste0(temp_summary$`year(date)`,
                                                 as.character(str_pad(temp_summary$`month(date)`, 2, pad = "0")),
                                                 as.character(str_pad(temp_summary$`day(date)`, 2, pad = "0"))),
                                          format = "%Y%m%d", tz = "GMT")), .before = 1) %>%
    return(temp_summary) # Returning daily temp summary data frame
}
